require 'rails_helper'RSpec.feature 'User posts index page', type: :feature do
  let(:user) { create(:user) }
  let!(:posts) { create_list(:post, 10, author: user) }  scenario 'displays user information and posts' do
    visit_user_posts_page    expect_user_information_displayed
    check_display_for_each_post    expect_pagination_button
    click_pagination_button
    expect_redirected_to_user_posts_path
  end  scenario 'displays the first comments on a post' do
    visit_user_posts_page    user.posts.each_with_index do |post, index|
    ...
    visit_user_posts_page    click_first_post    expect_redirected_to_post_show_page
  end  private  def visit_user_posts_page
    visit user_posts_path(user)
  end  def expect_user_information_displayed
